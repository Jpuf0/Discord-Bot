...
for (const cmd in commands) {
  if (result[cmd]) global.logger.error(`Unable to register commmand ${cmd}, a command with this name already exists.`, true)
...
async function helpingHand (user, context, cmd) {
  ...
  if (commands[cmd]) {
    ...
    if (context.guild) {
      ...
      context = await global.bot.users.get(user).getDMChannel()
    }
