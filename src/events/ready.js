const encoder = require('../selectors/encoder-selector')
module.exports = async function () {
  global.logger.log(`Fully Ready, Serving ${global.bot.guilds.size} guild(s).`)
  if (!global.bot.bot) global.logger.warn("You're not using a bot account to run Yui, this is unsupported and could cause problems.")
  ...
  if (!process.env.YUI_DISABLE_MUSIC) encoder.init()
}
