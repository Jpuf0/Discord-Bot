...
module.exports = {
  ...
  fn: async (msg, suffix) => {
    if (global.bot.voiceConnections.get(msg.channel.guild.id)) {
      if (!suffix) {
        global.i18n.send('VOLUME_NO_SUFFIX', msg.channel, { _volume: guildInfo[msg.channel.guild.id].volume })
        ...
        global.i18n.send('VOLUME_ADJUSTED', msg.channel, { _volume: suffix })
      }
    } else {
